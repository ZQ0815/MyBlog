...
4.  客户端收到服务器端的证书之后，客户端通过操作系统中已内置的公钥检查证书其合法性，如果发现发现证书有问题，那么HTTPS传输就无法继续，如果证书合格，则客户端会生成一个客户端密钥（pre-master 随机密码串），然后用服务器的公钥对客户端密钥进行非对称加密成密文，至此，HTTPS中的第一次HTTP请求结束；
